...
const parseRLPToPegoutRequests = (rlp) => {
  ...
  for (let i = 0; i < rlpPegoutRequests.length / 3; i++) {
    const destinationAddressHash160 = Buffer.from(
      rlpPegoutRequests[i * 3],
      "hex"
    ).toString("hex");
    const amountInSatoshis = new BN(rlpPegoutRequests[i * 3 + 1]).toString();
    const rskTxHash = Buffer.from(rlpPegoutRequests[i * 3 + 2], "hex").toString(
      "hex"
    );

    pegoutRequests.push(
      new PegoutRequest(destinationAddressHash160, amountInSatoshis, rskTxHash)
    );
  }
